...
1. 按天或按周检查账号、SSH、敏感权限、端口、防火墙、内核加固、软件包更新与 Docker Socket 风险；同时以秒级传感器持续观察认证、账号/持久化、网络、特权进程和可选的 Falco 运行时事件；
2. 把扫描发现和运行时信号归一化、关联为有生命周期的安全事件，保留证据与时间线；
3. 用户开启对应能力后，常驻 AI 安全工程师只在出现新证据时使用固定的只读调查工具分析事件；节省/平衡/敏锐三档、每日预算和严重事件专用储备共同限制 Token 消耗；
...
未配置 AI 时，规则扫描、持续信号、事件关联、证据、风险等级和人工修复工作流仍可使用。配置后，可按设备和能力分别开启“AI 协助”或“增强自动化”；后台 worker 只会读取白名单化、限量后的设备态势、事件信号、当前 Finding、近期动作、策略边界、最近巡检、事件时间线与同类事件。模型必须分别输出直接事实、推断、未知项和下一步只读检查，且不能调用 Agent、Helper 或 shell。密钥由本机主密钥加密后写入 SQLite，主密钥文件权限为 `0600`，UI 与日志不会回显完整密钥。参见 [AI 提供方配置](docs/ai-providers.md)和[常驻 AI 安全工程师](docs/security-engineer.md)。
...
- 自动动作必须可逆、带 TTL、频率限制、精确目标重验和紧急停止；网络动作还必须有管理员白名单；
...
- [x] 账号/权限、关键文件、计划任务、网络监听、关键进程、systemd 服务和容器配置的持续变化信号
