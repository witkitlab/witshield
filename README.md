...
3. 把修复转换成结构化计划，预览影响，等待用户批准；
4. 执行前留存状态，执行后重新验证，失败时尝试回滚；
5. 对用户明确预授权的低风险场景，按限时策略自动遏制并留下完整审计记录。
...
未配置 AI 时，规则扫描、证据、风险等级和修复工作流仍可使用；AI 主要负责解释。密钥由本机主密钥加密后写入 SQLite，主密钥文件权限为 `0600`，UI 与日志不会回显完整密钥。参见 [AI 提供方配置](docs/ai-providers.md)。
